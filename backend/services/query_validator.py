...
BLOCKED_PATTERNS = [
    r"\bimport\b",
    r"(?:^|\n)\s*from\s+\w+\s+import\s",  # Only Python import syntax, not column names like 'from_date'
...
def validate_syntax(code: str) -> Tuple[bool, str]:
    ...
    """
    if not code:
        return False, "No code was generated by the LLM"
    try:
        ast.parse(code)
        return True, ""
        ...
        return False, "Code generation returned an invalid type (expected string)"
...
def validate_schema_consistency(code: str, column_names: List[str]) -> Tuple[bool, List[str]]:
    ...
    for literal in string_literals:
        ...
        # Extended exclusion set — common Pandas kwargs and agg names that are
        # NOT column references and should never produce a warning
        _PANDAS_KEYWORDS = {
            "sum", "mean", "count", "min", "max", "std", "median", "first", "last",
            "records", "index", "values", "split", "table", "tight",  # orient args
            "coerce", "ignore", "raise",                               # errors args
            "inner", "outer", "left", "right", "cross",               # merge how
            "any", "all",                                              # dropna how
            "iso", "epoch",                                            # date_format
            "auto", "D", "W", "M", "Y", "H",                         # freq strings
            "ffill", "bfill", "pad",                                  # fill methods
            "spearman", "pearson", "kendall",                          # corr methods
            "dense", "min", "max", "average", "first",               # rank methods
            "numeric_only",                                            # kwarg names
        }
        if (
            literal not in column_names
            and literal not in ALLOWED_OPERATIONS
            and literal not in _PANDAS_KEYWORDS
